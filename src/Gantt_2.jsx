...
function isWorkDay(isoStr, cfg){
  ...
  if(skipWeekends){
    ...
    if(cfg.skipFri && dow===0) return false; // legacy migration
  }
...
const Gantt = React.forwardRef(function Gantt({ rows, setRows, zoom='days', cp=new Set(), allRows, levelMap, childrenMap, rowsMap, addSubTask, toggleCollapse, showCriticalOnly, showDependencies=false, calendarCfg }, ref){
  ...
  const {tStart, totalDays, tw, pxPerDay, months, x, todayX, nonWorkDays} = useMemo(()=>{
    ...
    const starts = rows.map(r=>r.start).filter(s=>s && /^\d{4}-\d{2}-\d{2}$/.test(s)).sort();
    const ends = rows.map(r=>r.finish).filter(s=>s && /^\d{4}-\d{2}-\d{2}$/.test(s)).sort();
    if(!starts.length || !ends.length){
      // No valid dates - show empty timeline but keep rows
      ...
      const xFn = (v) => {
        if(!v || !/^\d{4}-\d{2}-\d{2}$/.test(v)) return NaN;
        return dayDiff(s, v) * (w/days);
      };
      return {tStart:s, totalDays:days, tw:w, pxPerDay:px, months:[{key: new Date().toLocaleDateString('en-US',{month:'long',year:'numeric'}), start:0, count:days}], x:xFn, todayX:0, nonWorkDays:[]};
    }
  ...
  if(!rows?.length) return (
    <div className="emptyGantt">
      ...
      <div className="emptyHint">Tip: Use Quick Templates for 1-click setup • Drag grip ↕ to reorder in List • {cfg.enabled?'Fri/Sat/Holidays shaded':'All days enabled'}</div>
    </div>
  );
  ...
  return (
    <div className="ganttShell clickup" ref={ref}>
      ...
      {cfg.enabled && <div className="calendarLegend"><span className="calDot weekend"></span>Fri/Sat • <span className="calDot holiday"></span>Federal Holiday • <span className="calCount">{nonWorkDays.length} non-work days in range</span></div>}
      <div className="ganttViewport" ref={viewportRef} onScroll={onScroll}>
        <div className="ganttCanvas" style={{width:W+tw, height: HH + rows.length*RH}}>
          <div className="ganttHeader" style={{width:W+tw}}>
            <div className="ganttTaskHeader" style={{width:W}}>Task • {totalDays}d • {zoom} • {Math.round(tw)}px {showCriticalOnly?'• Critical only':''} {cfg.enabled?'• Skip Fri/Sat/Hols':''}</div>
              ...
              return (
                <div key={r.uid} className={`ganttRow ${isDimmed?'dimmed':''}`} style={{height:RH, top:HH+actualIdx*RH, position:'absolute', left:0, width:W+tw, opacity: isDimmed?0.35:1, zIndex:2}}>
                  ...
                  <div className="ganttTimeline" style={{left:W,width:tw}}>
                    {r.start && /^\d{4}-\d{2}-\d{2}$/.test(r.start) && !isNaN(x(r.start)) ? (
                    <div onContextMenu={e=>handleContextMenu(e,r)} onPointerDown={e=>onDragStart(e,r)} className={`ganttBubble ${isCritical?'critical':''} ${r.parentUid ? 'subtask-bubble' : ''} ${r.allowNonWork?'allow-bubble':''}`} style={{left:x(r.start),width:Math.max(tw/totalDays, r.duration*(tw/totalDays)), background: isCritical? '#FF6A35' : phaseCol}}>
                      ...
                      <span className="bubbleName">{r.name}{r.allowNonWork?' ⚠️':''} {r.percentComplete>0 ? `(${r.percentComplete}%)` : ''}</span><span className="bubbleDur">{r.duration}d</span><div style={{position:'absolute',left:0,bottom:0,height:4,width:`${r.percentComplete||0}%`,background:'#16A34A',opacity:0.8}}></div>
                      ...
                      <div className="ganttNoDate" style={{left:4, top:12, fontSize:10, color:'#94a3b8', fontStyle:'italic'}}>No date - set in List view</div>
                    )}
                  </div>
                </div>
              )
      ...
      {ctxMenu && (
        <div className="ganttCtxMenu" style={{left:ctxMenu.x, top:ctxMenu.y}}>
          ...
          <button onClick={()=>{ setRows(prev=>prev.map(r=>r.uid===ctxMenu.row.uid?{...r, allowNonWork:!r.allowNonWork}:r)); setCtxMenu(null); }}>{ctxMenu.row.allowNonWork?'✅ Remove Override':'⚠️ Allow Fri/Sat/Holiday'} <kbd>O</kbd></button>
